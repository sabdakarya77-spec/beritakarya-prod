...
export function GSCTopQueries({ data }: GSCTopQueriesProps) {
  if (!data || data.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-gray-200 p-6 text-center dark:border-white/10">
        <p className="text-sm text-brand-text-muted">Data query tidak tersedia</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white dark:border-white/5 dark:bg-white/[0.02]">
      <div className="border-b border-gray-100 px-5 py-3 dark:border-white/5">
        <h3 className="text-sm font-bold text-brand-black dark:text-white">Top Search Queries</h3>
      </div>
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-white/5">
            {data.map((row, idx) => (
              <tr key={idx} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                ...
                <td className="max-w-[200px] truncate px-5 py-2.5 font-medium text-brand-black dark:text-white">
                  {row.query}
                </td>
